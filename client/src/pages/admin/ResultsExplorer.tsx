...
export const ResultsExplorer: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      const [subsRes, usersRes, testsRes] = await Promise.all([
        api.getSubmissions({ userId: selectedUserFilter || undefined, testId: selectedTestFilter || undefined }),
        api.getAllUsers(),
        api.getTests(),
      ]);
  ...
  useEffect(() => {
    loadData();
  }, [selectedUserFilter, selectedTestFilter]);
  ...
  const filteredSubmissions = submissions.filter((sub) => {
    ...
    return athleteName.toLowerCase().includes(q) || athleteEmail.toLowerCase().includes(q) || testName.toLowerCase().includes(q);
  });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-rose-200 mt-1">
          Supervisa en detalle cada decisión que tomó cada atleta en las evaluaciones tácticas.
        </p>
      </div>
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-3xl p-5 border border-white/15 shadow-xl grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Buscar por atleta o evaluación..."
                    ...
                    <td className="py-4 px-6 font-semibold text-white">
                      <div className="flex items-center space-x-2.5">
                        <img
                          ...
                          alt="avatar"
                          className="w-7 h-7 rounded-full border border-white/30 object-cover bg-stone-900 flex-shrink-0"
                          onError={(e) => {
                            (e.target as HTMLImageElement).src = '/icons/icondefault.png';
                          }}
                        />
                        <div>
                          <p className="font-bold">{sub.user?.name || sub.userName}</p>
                          <p className="text-[10px] text-stone-400">{sub.user?.email || sub.userEmail}</p>
                        </div>
                      </div>
                    </td>
